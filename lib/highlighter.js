...
//	var MAX_LINES_PER_PASS = 800;
	...
	// When parsing in response to a scroll event, we start from viewportIndex - VIEWPORT_BUFFER_LINES to preempt
	// small amounts of upward-scrolling.
	var VIEWPORT_BUFFER_LINES = 30;
	...
	Highlighter.prototype = {
		...
		setViewportIndex: function(viewportIndex, viewportBottomIndex) {
			this.viewportIndex = Math.max(viewportIndex - VIEWPORT_BUFFER_LINES, 0);
			this.viewportBottomIndex = Math.min(viewportBottomIndex, this.model.getLineCount() - 1);
			if (!(this.lines[viewportIndex] && this.lines[viewportIndex].eolState)) {
				// Kick off a highlight job immediately to handle the viewport region
				this.highlightJob();
			}
		},
...
//			console.debug("Redrawing " + startLine + " .. " + endLine);
		...
		highlightJob: function() {
			...
			while (this.dirtyLines.length) {
				var viewportIndex = this.viewportIndex, viewportBottomIndex = this.viewportBottomIndex;
				var viewportLine = this.lines[viewportIndex], isViewportCase = viewportLine && !viewportLine.eolState, line;
				if (isViewportCase) {
					line = viewportIndex;
					//console.debug("Start from viewport " + line);
				} else {
				...
				for (var i=startIndex; i < lineCount; i++) {
					...
					var timeElapsed = +new Date() > stopTime;
					var endOfViewport = isViewportCase && i > (viewportBottomIndex + VIEWPORT_BUFFER_LINES);
					// If we're in the viewport case, stop after finishing viewport, so we'll return viewport region's style faster
					if (workRemains && (timeElapsed || endOfViewport)) {
						//endOfViewport && console.debug("endOfViewport at " + i + "; stopping pass.");
						// Stop, continue later
						//this.expandRedrawRange(startIndex, i + 1);
