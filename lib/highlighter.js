...
define(["orion/codeMirror", "orion/textview/eventTarget"], function(mMirror, mEventTarget) {
	 ...
	 * @class
	 */
	function Highlighter(model, modeSpec, whitespacesVisible) {
	...
	Highlighter.prototype = {
		_onModelChanged: function(e) {
			...
			var model = this.model, start = e.start, startLine = model.getLineAtOffset(start);
...
//			this.dirtyLines = dl;
		...
		loadMode: function() {
			this.mode = mMirror.getMode(mMirror.options, this.modeSpec);
		...
		highlight: function() {
			...
			while (this.dirtyLines.length) {
				...
				state = state ? mMirror.copyState(this.mode, state) : this.mode.startState();
				for (var i=startIndex, numUnchanged = 0; i < lineCount; i++) {
					...
					line.eolState = mMirror.copyState(this.mode, state);
					if (isChanged) {
		...
		styleReady: function() {
			if (this.redrawStart < Number.MAX_VALUE && this.redrawEnd > -1) {
				for (var i=this.redrawStart; i <= this.redrawEnd; i++) {
					...
					var ranges = line && this.styleToRange(line.style, lineStart);
					if (ranges && ranges.length) {
						var event = {
							type: "StyleReady",
							// other junk
						}
						this.dispatchEvent(styleReadyEvent);
					}
				}
