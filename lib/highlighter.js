...
define(["CodeMirror", "orion/textview/eventTarget"], function(mMirror, mEventTarget) {
	...
	// When we have styling for more than this many lines, we'll stop and schedule a follow-up pass.
	// (Avoids swamping the postmessage bus)
	var MAX_LINES_PER_PASS = 500;
		...
		highlight: function() {
			...
			while (this.dirtyLines.length) {
				...
				for (var i=startIndex, numUnchanged = 0; i < lineCount; i++) {
					...
					var tooManyLines = this.getRedrawExtent() > MAX_LINES_PER_PASS;
					if (timeElapsed || tooManyLines) {
						// Stop, continue later
